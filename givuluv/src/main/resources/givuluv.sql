...
select * from seller;
update seller set sellerpw = 'Asdf1234!' where sellerid = 's_banana';
update seller set sellerpw = 'Zxcv1234!' where sellerid = 's_cherry';
drop table seller;
CREATE TABLE `org` (
...
insert into store values (0,'상점이름3',now(),'010-1234-1234','zipcode','addr','addrdetail','etc',now(),'이체리','s_cherry');
...
select * from product;
INSERT INTO product (productname, p_amount, cost, connectid, p_type) VALUES ('할미할배20', 500, 5000, '4', 'm');
delete from product where connectid = 2;
